            ...
            <div class="details_content container">
                <div class="application_detail">
                    ...
                    <div style="padding-left: 20px;">
                        ...
                        <h6>{{ $t('confirm_application_name_description') }} <span class="category">{{
                                confirmData.app_name
                        }}</span>.</h6>
                        <h6>{{ $t('confirm_application_categorytab_description') }} <span class="category">{{
                                confirmData.category_tab
                        }}</span>.</h6>
                        <h6>{{ $t('confirm_application_capacity_description') }} <span class="category">{{
                                confirmData.capacity
                        }}{{ confirmData.capacity_unit }}</span>.</h6>
                        <h6>{{ $t('confirm_application_period_description') }} <span class="category">{{
                                confirmData.period_date
                        }} {{ $t('days') }}</span>.</h6>
                    </div>
                </div>
                ...
                <div class="transaction_detail">
                    ...
                    <div style="padding-left: 20px;">
                        ...
                        <h6>{{ $t('confirm_transaction_id_description') }} <span class="category">{{
                                confirmData.transaction_id
                        }}</span>
                            {{ $t('confirm_transaction_create_date_description') }} <span class="category">{{
                                    confirmData.created_at
                            }}</span>.</h6>
                        <h6>{{ $t('confirm_transaction_price_description') }} <span class="category">{{
                                $t(confirmData.transaction_price)
                        }}¥</span>.</h6>
                    </div>
                </div>
            </div>
...
export default {
    ...
    created() {
        var transID = this.$route.query.payment_intent == undefined ? this.$route.query.transaction_id : this.$route.query.payment_intent

        // Save stripe transaction info into DB
        axios.post('/api/payment/stripe/checkout/transaction/save', {
            transID: transID
        }).then((res) => {
            console.log('success')
        })
    ...
    methods: {
        ...
        async getconfirmDataByTransId(id) {
            ...
            // test_app_url
            // if (this.confirmData.app_url == undefined) {
            //     console.log("app_url is undefiend")
            // }
        },
        async request_to_app() {
            console.log("is this request_to_app.", this.confirmData)
