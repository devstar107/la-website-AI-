      ...
      <div class="content">
            ...
            <div class="form-group">
              <form id="komoju-form" method="post">
                <input
                  ...
                  name="komojuToken"
                />
                <b-button variant="outline-primary" id="bank_transfer"
                  >{{ $t("bank_transfer") }}
                </b-button>
                <!-- <b-button variant="outline-primary" id="debit_card">{{ $t('debit_card') }}</b-button> -->
                <b-button variant="outline-primary" id="paypal">{{
                  $t("paypal")
                }}</b-button>
                <b-button variant="outline-primary" id="amazon_pay">{{
                  $t("amazon_pay")
                }}</b-button>
                <b-button variant="outline-primary" id="credit_card">{{
                  $t("credit_card")
                }}</b-button>
                <a href="/gmo/auth" target="_blank">
                  <b-button variant="outline-primary" id="gmo_transfer">{{
                    $t("GMO")
                  }}</b-button>
                </a>
              </form>
        ...
        <div class="form-group float-right">
          <b-button
            ...
            :to="{ name: $t('purchase.url') }"
            >{{ $t("back") }}</b-button
          >
          <b-button variant="outline-primary" id="subscribe">{{
            $t("subscribe")
          }}</b-button>
        </div>
      </div>
  ...
  methods: {
    async getCategorydata(app_id, cat_id) {
      const { data } = await axios.get(
        "/api/get/checkout/category/" + app_id + "/" + cat_id
      );
      this.category = data;
    ...
    /**
     * 20220614_stripe card payment_darbinyan.dev
     */
    stripeCheckout(that) {
      ...
      async function handleSubmit(e) {
        ...
        const { error } = await stripe.confirmPayment({
          ...
          confirmParams: {
            ...
            return_url: "http://localhost:8008/checkout/confirmation",
          ...
          onApprove: function (data, actions) {
            ...
            return actions.order.capture().then(function (orderData) {
              ...
              if (orderData.status == "COMPLETED") {
                axios
                  .post("/api/payment/checkout/transaction/save/", {
                    transaction:
